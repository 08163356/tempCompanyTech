...
```
          <section class="banner">
            <section
              class="p3"
              v-if="item.type === 'ai' && dialogueList.length - 1 === index"
            >
              <span>{{ latestAiMsg }}</span>
            </section>
```
...
const printCharacter = (
  latestAiMsg: Ref<UnwrapRef<string>>,
  tempStrForLatestMsg: Ref<UnwrapRef<string>>
) => {
  if (latestAiMsg.value.length >= tempStrForLatestMsg.value.length) {
    return
  } else {
    // latestAiMsg仍然比tempStrForLatestMsg短，进入下一次等待和递归
    setTimeout(() => {
      latestAiMsg.value += tempStrForLatestMsg.value.charAt(
        latestAiMsg.value.length
      )
      printCharacter(latestAiMsg, tempStrForLatestMsg)
    }, 20)
  }
}

// function callback(timestamp: number) {
//   if (!lastTime) lastTime = timestamp // 初始化 lastTime
//   const progress = timestamp - lastTime // 计算距离上次调用的时间差
//
//   if (progress >= 1000) {
//     console.log('这段代码每秒执行一次')
//     lastTime = timestamp // 更新 lastTime
//   }
//
//   requestAnimationFrame(callback) // 继续请求下一个动画帧
// }

watch(dialogueList.value, (nv) => {
  if (nv.length === chatLen.value) {
    return
  }
  latestAiMsg.value = ''
  tempStrForLatestMsg.value = ''
  let newMsg = nv.slice(-1)[0]
  if (newMsg.type === 'ai') {
    tempStrForLatestMsg.value = newMsg.content
    printCharacter(latestAiMsg, tempStrForLatestMsg)
    chatLen.value = nv.length
  }
})
```
